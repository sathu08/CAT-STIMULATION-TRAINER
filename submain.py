...
def user_page():
    ...
    return render_template("user_home_screen.html", user_name=current_user.username)
    ...
    if request.method == "GET":
        ...
        for language_tuple in languages:
            ...
            if language not in seen_words:
                seen_words.add(language)
                unique_words.append(language)
        return render_template("take_test.html", languages=unique_words, user_name=current_user.username,
                               model=model)
    if request.method == "POST":
        ...
        if test_detail:
            ...
            img = cv2.imread('static/second_folder/%s' % test_detail[1])
            ...
            return render_template('take_test.html', test_details=test_detail, file_name=file_name,
                                   user_name=current_user.username, model=model)
        else:
